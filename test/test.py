...
class SshRunPrivateKeyTestCase(PipeTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.private_key_image = cls.docker_client.images.build(path=dirname, dockerfile=os.path.join(dirname, 'Dockerfile'), tag=cls.test_image_name)

    def setUp(self):

        self.api_client = self.docker_client.api
        self.ssh_key_file_container = self.docker_client.containers.run(self.test_image_name, detach=True)
